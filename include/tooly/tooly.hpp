 ...
 */
template <typename T, size_t N, typename U, typename F, size_t window_size>
auto transform_window(std::span<T, N> collection, F op) {
  // Check that there are at least window_size elements in the collection
  if constexpr (window_size > N) {
    return {};
  } 
  ...
  // adjacent_difference?
  std::array<U, N - window_size + 1> output;
  auto window = std::span<T, window_size>{collection.begin(), collection.begin() + window_size};
  // while (view.begin() != std::prev(end)) {
